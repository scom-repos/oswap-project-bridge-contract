...
contract OSWAP_MainChainTrollRegistry is Authorization, ERC721Holder, ReentrancyGuard {
    ...
    function getTrollProfileByAddress(address trollAddress) external view returns (
        TrollProfile memory troll
    ) {
        troll = trollProfiles[trollProfileInv[trollAddress]];
    }
    ...
    function stakeTroll(uint256 trollProfileIndex, I_TrollNFT nft, uint256 tokenId) external nonReentrant whenNotPaused {
        require(trollProfiles[trollProfileIndex].trollType == nftType[nft], "Invalid type");
        (uint256 stakes) = _stakeMainChain(trollProfileIndex, nft, tokenId);
        emit StakeTroll(msg.sender, trollProfileIndex, nft, tokenId, stakes, stakeOf[msg.sender]);
    }

    // add more stakes to the specified nft/tokenId
    function _addStakesTroll(I_TrollNFT nft, uint256 tokenId, uint256 amount) internal returns (uint256 trollProfileIndex){
    ...
    function addStakesTroll(I_TrollNFT nft, uint256 tokenId, uint256 amount) external nonReentrant whenNotPaused {
        uint256 trollProfileIndex = _addStakesTroll(nft, tokenId, amount);
        require(trollProfiles[trollProfileIndex].trollType == nftType[nft], "Invalid type");
        emit StakeTroll(msg.sender, trollProfileIndex, nft, tokenId, amount, stakeOf[msg.sender]);
    }
    ...
    function unstakeTroll(I_TrollNFT nft, uint256 tokenId) external nonReentrant whenNotPaused returns (uint256 trollProfileIndex) {
        uint256 stakes;
        (trollProfileIndex, stakes) = _unstakeMainChain(nft, tokenId);
        require(trollProfiles[trollProfileIndex].trollType == nftType[nft], "Invalid type");
        emit UnstakeTroll(msg.sender, trollProfileIndex, nft, tokenId, stakes, stakeOf[msg.sender]);
    }
